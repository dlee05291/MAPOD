...
%        'ampNoise' - noise amplitude [6] (optional)
%        'ahatSat' - saturation amplitude [inf] (optional)
%        'ahatDec' - ahat decision [NaN] (optional)
%        'ahatDecFactor' - factor for ahat decision [0.2] (optional)
%        'p1' - slope of a vs ahat model [2.036] (optional)
%        'p2' - intercept of a vs ahat model [2] (optional)
...
% 030920 LDY Code was modified to conduct censored regression correctly.
%            Previous version has wrong been doing censored regression,
%            which was rather ordinary regression.

% Set default.
inspID = 1;            % Inspector ID
aData = NaN;           % Crack size
ahatData = NaN;        % Signal amplitude
logX = 1;              % Log transformation of x axis
logY = 1;              % Log transformation of y axis
ampNoise = 6;          % Noise amplitude
ampSat = inf;          % Saturation amplitude
ahatDec = 14;          % ahat decision
ampDecFactor = 0.2;    % Factor for decision amplitude
p1 = 2.213;            % Slope of a vs ahat model (refer to 'getSpecimenRefData')
p2 = 2.12;             % Intercept of a vs ahat model
...
% Crack length
aData([5, 6, 22, 29]) = [1.42, 2.72, 2.34, 3.89];
...
% Calculate 'ahatDec' if necessary.
if isnan(ahatDec)
    if isinf(ampSat)
        ahatDec = (max(ahatData) - ampNoise)*ampDecFactor + ampNoise;
    else
